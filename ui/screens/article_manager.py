...
class ArticleManager(ctk.CTkFrame):
    """
    Interface de gestion des articles avec un design moderne, aligné avec StockManager.
    """
    ...
    def __init__(self, parent, **kwargs):
        ...
        print(f"DEBUG: ArticleManager initialized in {self.current_theme} mode")
    ...
    def refresh_data(self):
        """Recharge les données des articles et met à jour l'interface."""
        print("DEBUG: Refreshing article data")
        self._load_sample_data()  # Recharge les données d'exemple
        self._update_table()      # Met à jour le tableau
        
        # Mettre à jour le compteur d'alertes
        alert_count = sum(1 for item in self.article_data 
                         if 0 < item["stock"] <= item.get("seuil", 0))
        self.alert_counter.configure(text=f"{alert_count} articles en alerte")
    ...
    def _create_toolbar(self):
        ...
        for i, (filter_id, filter_text) in enumerate(filters):
            btn = ctk.CTkButton(
                filter_frame,
                text=filter_text,
                width=self.SIZES["filter_button_width"],
                height=self.SIZES["filter_button_height"],
                fg_color=self.COLORS[self.current_theme]["primary"] if filter_id == self.filter_type else "transparent",
                hover_color=self.COLORS[self.current_theme]["primary_hover"] if filter_id == self.filter_type else self.COLORS[self.current_theme]["table_selected"],
                text_color=self.COLORS[self.current_theme]["bg"] if filter_id == self.filter_type else self.COLORS[self.current_theme]["text"],
                border_width=1,
                border_color=self.COLORS[self.current_theme]["border"],
                font=self.FONTS["label_bold"],
                command=lambda x=filter_id: self._apply_filter(x)
            )
        ...
        add_btn = ctk.CTkButton(
            ...
            text_color=self.COLORS[self.current_theme]["bg"],
            font=self.FONTS["button"],
        ...
        self.search_entry = ctk.CTkEntry(
            ...
            fg_color=self.COLORS[self.current_theme]["bg"],
            text_color=self.COLORS[self.current_theme]["text"],
    ...
    def _create_article_table(self):
        ...
        print("DEBUG: Creating article table")
        ...
        style.map("Custom.Treeview.Heading",
                background=[("active", self.COLORS[self.current_theme]["table_selected"])])
        
        style.configure(
            "Custom.Treeview",
            background=self.COLORS[self.current_theme]["bg"],
            foreground=self.COLORS[self.current_theme]["text"],
            fieldbackground=self.COLORS[self.current_theme]["bg"],
            borderwidth=0,
        ...
        columns = ("code", "designation", "categorie", "prix_achat", "prix_vente", "stock", "seuil", "statut", "actions")
        ...
        self.table.column("code", width=120, minwidth=100, anchor="w")
        self.table.column("designation", width=250, minwidth=200, anchor="w")
        self.table.column("categorie", width=150, minwidth=120, anchor="w")
        self.table.column("prix_achat", width=120, minwidth=100, anchor="e")
        self.table.column("prix_vente", width=120, minwidth=100, anchor="e")
        self.table.column("stock", width=100, minwidth=80, anchor="center")
        ...
        self.table.heading("code", text="Code Article", command=lambda: self._sort_table("code"))
        ...
        self.table.heading("prix_achat", text="Prix d'achat (€)", command=lambda: self._sort_table("prix_achat"))
        self.table.heading("prix_vente", text="Prix de vente (€)", command=lambda: self._sort_table("prix_vente"))
        self.table.heading("stock", text="Stock", command=lambda: self._sort_table("stock"))
    ...
    def _create_analysis_chart(self):
        ...
        ctk.CTkLabel(
            chart_frame,
            text="📊 Aperçu des articles",
            font=self.FONTS["subtitle"],
            text_color=self.COLORS[self.current_theme]["text"]
        ).pack(pady=self.SIZES["small_padding"], padx=self.SIZES["small_padding"], anchor="w")
        
        in_stock = sum(1 for item in self.article_data if item["stock"] > item["seuil"])
        alert = sum(1 for item in self.article_data if 0 < item["stock"] <= item["seuil"])
        out_of_stock = sum(1 for item in self.article_data if item["stock"] == 0)
        
        # ... (rest of the code remains the same)
    
    def _update_table(self):
        """Met à jour le tableau avec les données actuelles des articles."""
        print("DEBUG: Updating article table")
        ...
        if self.sort_column:
            ...
            self.article_data.sort(key=lambda x: x.get(self.sort_column, ""), reverse=reverse)
        
        # Ajouter les données triées au tableau
        for item in self.article_data:
            # Déterminer le statut et le tag de style
            if item["stock"] == 0:
                status = "Rupture"
                tag = "out_of_stock"
            elif 0 < item["stock"] <= item.get("seuil", 0):
                status = "Stock bas"
                tag = "alert"
            else:
                status = "En stock"
            ...
            values = (
                ...
                item["designation"],
                item["categorie"],
                f"{item['prix_achat']:.2f} €",
                f"{item['prix_vente']:.2f} €",
                str(item["stock"]),
                str(item.get("seuil", 0)),
                status,
                "✏️  🗑️"
            )
            
            # Ajouter la ligne au tableau avec le tag approprié
            self.table.insert("", "end", values=values, tags=(tag,))
    
    def _load_sample_data(self, tree=None):
    ...
    def _update_table(self):
        ...
        filtered_data = self._apply_filters(self.article_data)
    ...
    def _apply_filters(self, data):
        ...
        if self.filter_type == "all":
            ...
            return [item for item in data if item["stock"] > item["seuil"]]
        elif self.filter_type == "alert":
            return [item for item in data if 0 < item["stock"] <= item["seuil"]]
        elif self.filter_type == "out_of_stock":
            return [item for item in data if item["stock"] == 0]
        return data
    
    def _update_counters(self, data):
        """Met à jour les compteurs d'alertes."""
        alert_count = sum(1 for item in data if 0 < item["stock"] <= item["seuil"])
        alert_count = sum(1 for article in self.article_data if article["stock"] <= article["seuil"])
        self.alert_counter.configure(text=f"{alert_count} articles en alerte")
        
        # Mise à jour des couleurs des boutons de filtre
    ...
    def _search_items(self, event=None):
        ...
        filtered_data = [
            item for item in self.article_data
...
def _show_add_article_form(self):
    """Affiche la boîte de dialogue pour ajouter un article."""
    ...
    fields = [
        ("code", "Code Article", "text"),
        ("designation", "Désignation", "text"),
        ("categorie", "Catégorie", "combobox"),
    ...
    for field, label, ftype in fields:
        ...
        if ftype == "combobox":
            entry = ctk.CTkComboBox(
                ...
                text_color=self.COLORS[self.current_theme]["text"],
                dropdown_fg_color=self.COLORS[self.current_theme]["fg"],
                button_color=self.COLORS[self.current_theme]["primary"],
                button_hover_color=self.COLORS[self.current_theme]["primary_hover"],
                border_width=1,
                border_color=self.COLORS[self.current_theme]["border"]
            )
        else:
    ...
    ctk.CTkButton(
        ...
        text="Annuler",
        fg_color=self.COLORS[self.current_theme]["secondary"],
        hover_color=self.COLORS[self.current_theme].get("secondary_hover", self.COLORS[self.current_theme]["secondary"]),
        font=self.FONTS["button"],
        text_color=self.COLORS[self.current_theme]["text"],
    ...
    ctk.CTkButton(
        ...
        text="Enregistrer",
        fg_color=self.COLORS[self.current_theme]["primary"],
        hover_color=self.COLORS[self.current_theme].get("primary_hover", self.COLORS[self.current_theme]["primary"]),
        font=self.FONTS["button"],
        text_color=self.COLORS[self.current_theme]["text"],
        command=lambda: self._save_new_item(entries, dialog)
    ).pack(side="right")

def _show_edit_article_form(self, code):
    """Affiche la boîte de dialogue pour modifier un article."""
    print(f"DEBUG: Showing edit dialog for {code}")
    article = next((item for item in self.article_data if item["code"] == code), None)
    if not article:
        messagebox.showerror("Erreur", "Article introuvable.")
        return

    dialog = ctk.CTkToplevel(self)
    dialog.title(f"Modifier l'article {code}")
    dialog.geometry(f"{self.SIZES['dialog_width']}x{self.SIZES['dialog_height']}")
    dialog.resizable(False, False)
    dialog.configure(fg_color=self.COLORS[self.current_theme]["bg"])
    dialog.grab_set()
    ...
    ctk.CTkLabel(
        dialog,
        text=f"Modifier l'article: {code}",
        font=self.FONTS["dialog_title"],
        text_color=self.COLORS[self.current_theme]["text"]
    ).pack(pady=self.SIZES["small_padding"])
    ...
    fields = [
        ("code", "Code Article", "text", article["code"]),
        ("designation", "Désignation", "text", article["designation"]),
        ("categorie", "Catégorie", "combobox", article["categorie"]),
        ("prix_achat", "Prix d'achat (€)", "number", str(article["prix_achat"])),
        ("prix_vente", "Prix de vente (€)", "number", str(article["prix_vente"])),
        ("stock", "Stock", "number", str(article["stock"])),
    ...
    ctk.CTkButton(
        ...
        hover_color=self.COLORS[self.current_theme].get("danger_hover", self.COLORS[self.current_theme]["danger"]),
        font=self.FONTS["button"],
        text_color=self.COLORS[self.current_theme]["text"],
        command=lambda: self._confirm_delete(article["code"], dialog)
    ...
    ctk.CTkButton(
        ...
        font=self.FONTS["button"],
        text_color=self.COLORS[self.current_theme]["text"],
        command=lambda: self._save_edited_item(article["code"], entries, dialog)
    ).pack(side="right")
    ...
    def _save_new_item(self, entries, dialog):
        ...
        new_item = {
            ...
            "prix_achat": float(entries["prix_achat"].get()),
            "prix_vente": float(entries["prix_vente"].get()),
            "stock": int(entries["stock"].get()),
            "seuil": int(entries["seuil"].get()),
            "statut": "En stock"
        }
        
        if any(item["code"] == new_item["code"] for item in self.article_data):
            messagebox.showerror("Erreur", "Un code d'article identique existe déjà.")
            return
        
        new_item["statut"] = self._calculate_status(new_item)
        self.article_data.append(new_item)
        self._update_table()
        dialog.destroy()
        messagebox.showinfo("Succès", "L'article a été ajouté avec succès.")
    
    def _save_edited_item(self, old_code, entries, dialog):
        """Enregistre les modifications d'un article existant."""
        print(f"DEBUG: Saving edited item {old_code}")
        if not all(entry.get().strip() for field, entry in entries.items() if field != "code"):
            messagebox.showerror("Erreur", "Tous les champs sont obligatoires.")
            return
        
        index = next((i for i, item in enumerate(self.article_data) 
                     if item["code"] == old_code), None)
        
        if index is None:
            messagebox.showerror("Erreur", "Article introuvable.")
            return
        
        self.article_data[index].update({
            "designation": entries["designation"].get().strip(),
            "categorie": entries["categorie"].get().strip(),
            "prix_achat": float(entries["prix_achat"].get()),
            "prix_vente": float(entries["prix_vente"].get()),
        ...
        self.article_data[index]["statut"] = self._calculate_status(self.article_data[index])
        self._update_table()
        dialog.destroy()
        messagebox.showinfo("Succès", "Les modifications ont été enregistrées avec succès.")
    
    def _confirm_delete(self, code, dialog):
        """Demande confirmation avant de supprimer un article."""
        print(f"DEBUG: Confirming delete for {code}")
        if messagebox.askyesno(
            "Confirmer la suppression",
            f"Êtes-vous sûr de vouloir supprimer l'article {code} ?",
            icon="warning"
        ):
            self._delete_item(code, dialog)
    
    def _delete_item(self, code, dialog):
        """Supprime un article de la liste."""
        print(f"DEBUG: Deleting item {code}")
        self.article_data = [item for item in self.article_data if item["code"] != code]
        self._update_table()
        dialog.destroy()
        messagebox.showinfo("Succès", "L'article a été supprimé avec succès.")
    
    def _calculate_status(self, item):
        """Calcule le statut d'un article en fonction de son stock et seuil."""
        if item["stock"] == 0:
            return "Rupture"
        elif item["stock"] <= item["seuil"]:
            return "En alerte"
    ...
    def _on_item_double_click(self, event):
        ...
        if item:
            ...
            if values:
                self._show_edit_article_form(values[0])
